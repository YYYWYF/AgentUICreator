...
export interface CreateAgentRuntimeOptions {
  ...
  mock?: MockAgentRuntimeConfig | undefined;
}
...
export class HttpAgentRuntime extends ObservableAgentRuntime {
  ...
  async sendMessage(input: string): Promise<void> {
    ...
    if (this.snapshot.isRunning || this.agent.isRunning) {
      throw new Error("The Agent Runtime is already processing a message.");
    }
...
export function createAgentRuntime(
  ...
  return endpoint === undefined || endpoint.length === 0
    ? new MockAgentRuntime(options.mock)
    : new HttpAgentRuntime({ endpoint });
}
