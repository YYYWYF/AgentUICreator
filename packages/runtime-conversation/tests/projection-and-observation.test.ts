...
import { projectConversationExecutions } from "../src/compatibility/conversation-execution-projector.js";
import { projectConversationMessages } from "../src/compatibility/conversation-message-projector.js";
import { ConversationObservationSourceImpl } from "../src/observation/conversation-observation-source.js";
...
describe("assistant-ui compatibility projection", () => {
  it("projects text, reasoning, and multiple tool results from Runtime state", () => {
    ...
    expect(projectConversationMessages(messages).map((message) => message.role))
      .toEqual(["reasoning", "assistant", "tool", "tool"]);
    expect(projectConversationExecutions(messages)).toMatchObject([
  ...
  it("exposes idle, running, completed, and error tool observation", () => {
    ...
    const source = new ConversationObservationSourceImpl(thread, () => "thread-1");
