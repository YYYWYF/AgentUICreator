...
- `usePluginEvents()` is the inbound, live-only Application Event channel. A Plugin may subscribe only to names declared in its manifest and registered by `/project/agent-contract/agent-events.ts`.
