...
2. For AppUIModel errors, check schema invariants, ids, `sizes`, Panel bounds, application plugins, Layout Slot `plugins`, and nested plugin-local Slots. Use `inspect_ui_slots` for authoring targets and current child arrays; Runtime slot ids are compiler output, not repair inputs.
