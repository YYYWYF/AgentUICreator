...
class UISourceCreationService:
    ...
    def _source_path(path: str) -> str:
        virtual = path
        if virtual.startswith("/project/"):
            virtual = virtual[len("/project") :]
        return virtual

    def _authorize(self, path: str) -> tuple[str, str]:
        try:
            normalized = self.policy.assert_write(self._source_path(path))
        ...
        for source in files:
            ...
            if receipt_path in seen:
                raise SourceCreationError(
                    ...
                    "create_ui_source_files contains duplicate normalized paths.",
                    {"path": receipt_path},
                )
