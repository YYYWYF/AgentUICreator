...
| AgentStatus | Existing runtime execution state only: `AgentStepExecution` and `AgentToolExecution` in `packages/runtime-core/src/agent-execution.ts`; no dedicated status semantic | `none` | `AgentStepExecution` has `id`, `producer`, `name`, `status`; tool lifecycle has `preparing|awaiting-result|completed|error|interrupted` | Existing execution lifecycle is not an explicit AgentStatus contract; no authoritative elapsed field is present | `dormant` | Do not derive labels from tool names or fabricate elapsed values. The mock projection derives state from `ConversationToolCallProps.status` and accepts label/elapsed only from Tool Args. |
