...
export const APP_UI_MODEL_VERSION = "1" as const;
...
export const slotNodeSchema: z.ZodType<SlotNode> = z.strictObject({
  ...
  pluginInstanceIds: z.array(nonBlankStringSchema),
});
...
export const appUIModelSchema = appUIModelShapeSchema.superRefine(
  (model, context) => {
    ...
    const mountedPluginInstances = new Set<string>();
    ...
    const visitNode = (node: LayoutNode, path: IssuePath): void => {
      ...
      if (node.type === "row" || node.type === "column") {
        if (node.sizes !== undefined && node.sizes.length !== node.children.length) {
          addIssue(
            context,
            [...path, "sizes"],
            "sizes must contain exactly one entry for each child",
          );
        }
      ...
      if (node.type === "stack") {
        if (
          ...
          !node.children.some((child) => child.id === node.active)
        ) {
          addIssue(
            context,
            [...path, "active"],
            `Stack active id "${node.active}" must reference a direct child`,
          );
        }
      ...
      if (node.type === "panel") {
        if (
          ...
          addIssue(
            context,
            [...path, "minWidth"],
            "minWidth must not be greater than maxWidth",
          );
        }
      ...
      if (slotIds.has(node.slotId)) {
        addIssue(
          context,
          [...path, "slotId"],
          `Duplicate slot id "${node.slotId}"`,
        );
      } else {
        slotIds.add(node.slotId);
      }

      const slotInstanceIds = new Set<string>();
      node.pluginInstanceIds.forEach((instanceId, index) => {
        const instancePath = [...path, "pluginInstanceIds", index];

        if (slotInstanceIds.has(instanceId)) {
          addIssue(
            context,
            instancePath,
            `Plugin instance "${instanceId}" is duplicated in this slot`,
          );
          return;
        }
        slotInstanceIds.add(instanceId);

        if (model.pluginInstances[instanceId] === undefined) {
          addIssue(
            context,
            instancePath,
        ...
        if (mountedPluginInstances.has(instanceId)) {
          addIssue(
            context,
            instancePath,
            `Plugin instance "${instanceId}" is mounted in more than one slot`,
          );
          return;
        }
        mountedPluginInstances.add(instanceId);
      });
    };
...
export function parseAppUIModelJson(json: string): AppUIModel {
  const input: unknown = JSON.parse(json);
  return parseAppUIModel(input);
}
