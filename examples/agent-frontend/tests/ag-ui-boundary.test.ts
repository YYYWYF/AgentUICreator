...
describe("generated app AG-UI dependency boundary", () => {
  ...
  it("keeps AG-UI SDK imports out of the generated app and runtime-core", async () => {
    ...
    for (const filename of (await Promise.all(
      ...
      for (const specifier of moduleReferences(await readFile(filename, "utf8"))) {
        ...
        if (filename.startsWith(`${coreRoot}${path.sep}`) && (
          ...
          violations.push(`${relative}: core imports conversation integration ${specifier}`);
        }
      }
    }
  ...
  it("enforces package dependency ownership and direction", async () => {
    ...
    const conversationPackage = await readPackage(path.join(conversationRoot, "package.json"));
    ...
    expect(dependencyNames(corePackage)).not.toContain("@agent-ui/runtime-conversation");
    expect(dependencyNames(corePackage).filter((name) => name.startsWith("@assistant-ui/"))).toEqual([]);
  ...
  it("uses only the runtime package public entry points across package boundaries", async () => {
    const violations: string[] = [];
    for (const filename of await sourceFiles(conversationRoot)) {
      const relative = path.relative(workspaceRoot, filename).split(path.sep).join("/");
      for (const specifier of moduleReferences(await readFile(filename, "utf8"))) {
        if (
